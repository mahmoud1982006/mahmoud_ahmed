...
## Part 1:
1-Firewall maintenance and using a more advanced firewall like Next GEN firewall
2-Password policies and forbid password sharing   
3-Multifactor authentication (MFA) 



## Part 2: 
1-Firewall maintenance : to block every port i don`t want keep only important ones monitor data packets and filter them before affecting the network  
  And it should implemented regularly like every 3 weeks to a month 
2-Password policies and forbid password sharing: the admin must change his password regularly can not stay on the default password 
And for admins the password should be changed every month
3-Multifactor authentication (MFA):so no one can access the account for ex even if one factor got compromised still at least one factor secure 
And it should be updated every 4 month or more
